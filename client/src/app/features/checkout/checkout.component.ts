...
export class CheckoutComponent implements OnInit, OnDestroy{
          ...
          const result = await this.stripeService.confirmPayments(this.confirmationToken);
           ...
           if(result.paymentIntent?.status === "succeeded"){
            ...
            if(orderResult){
              ...
              throw new Error('Order cretion failed');
            }
  ...
  private async createOrderModel(): Promise<OrderToCreate>{
    const cart = this.cartService.cart(); //get the product cart
    const shippingAddress = await this.getAddressFromStripeAddress() as ShippingAddress; //getting the address
    const card = this.confirmationToken?.payment_method_preview.card; //getting the card
      
    //check if we have cart , shipping and card details
     if(!cart?.id || !cart.deliveryMethodId || !card || !shippingAddress){
         throw new Error('Problem creating order');
     }

     //mapping just like dto createorder
    return {
      cartId: cart.id,
      paymentSummary: {
        last4: +card.last4,
